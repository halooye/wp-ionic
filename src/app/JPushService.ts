...
//
//   constructor(private events: Events, private jpush: JPush){
//
...
//       // this.setTags();
//       alert("极光推送初始化", {JPushResult: result});
//     }).catch(error => {
...
//       if(result == 0)
//         alert("系统设置中已关闭应用推送", {result: result});
//       if(result > 0)
//         alert("系统设置中打开了应用推送", {result: result});
//     });
//
...
//
//       alert("jpush.openNotification", {content: content});
//
...
//       // if( !content.has('type') || Utils.isEmptyStr(content.get('type')) ) content.set('type', NOTIFY_TYPE.MESSAGE);
//
//       alert("jpush.receiveNotification", {content: content});
...
//       // if( !content.has('type') || Utils.isEmptyStr(content.get('type')) ) content.set('type', NOTIFY_TYPE.MESSAGE);
//
//       alert("jpush.receiveMessage", {content: content});
...
//     }).catch(err => {
//       alert('极光推送设置标签setTags失败', err, {tags: tags});
//     });
//   }
//
//
